...
exports.getTableColumnsWithLabelFromMakeup = getTableColumnsWithLabelFromMakeup;
...
function getTableColumnsWithLabelFromMakeup(makeup_elements) {
  for (const item of makeup_elements) {
    if (item.type == 'table' && item.columns) {
      return {columns: item.columns, _label: item._label}
    }
  }
  return {};
}
...
function processMakeupElements(makeup_elements, { start, end }, data, reportVars) {
  ...
  elements.forEach(el => {
    ...
    if (el.type == 'table') {
      ...
      el.foot.forEach(item => {
        item.content = replaceMacro(item.content, true);
      });
  ...
  function replaceMacro(text, skipFormula) {
