...
module.exports = function(elements, tabledata, filename, mes) {
  let doc;
  let defaultFont;
  let tableFinalY;
  ...
  function build() {
    ...
    function addCircle(item) {
      ...
      const lineColor = getColor(item.boderColor);
      let style = 'S';
      ...
      doc.ellipse(item.x, item.y, item.r, item.h, style);
      */
     doc.setLineWidth(1);
     doc.setDrawColor(0);
     doc.ellipse(item.x, item.y, item.w, item.h, 'S');
/*
