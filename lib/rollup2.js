 ...
 */
module.exports = function rollup(arr, discrete, reportVars) {
  ...
  let vals = {}; // {meter1:22} - накапливаются данные строки
  ...
  reportVars.forEach(item => {
    ...
    } else if (item.col_type == 'dt') {
      item.calcFn = new Function('startTs, endTs', 'return ' + item.calc_row);
    }
  });
  ...
  while (j < arr.length) {
    if (curdtx == arr[j].dtx) {
      ...
      if (mapIndex[dn_prop]) {
        ...
        let varName = mapIndex[dn_prop].varname;
        if (vals[varName] == undefined) initVal(mapItem, varName, curval);
...
function dateFormat(one) {
  try {
    const dt1 = new Date(one.startTs);
    const dt2 = new Date(one.endTs);
    return dateutils.getDateTimeFor(dt1, 'reportdt')+'-'+dateutils.getDateTimeFor(dt2, 'reportdt');
  } catch (e) {
